...
export class BaseEnvironment extends EmitterObj {
  init(loaders, opts, Template) {
    ...
    if (!loaders) {
      throw new Error('Environment requires loaders');
    }
  ...
  getTemplate(name, eagerCompile, parentName, ignoreMissing, cb) {
    ...
    if (name instanceof this.Template) {
      tmpl = name;
    ...
    const createTemplate = (err, info) => {
      ...
      let newTmpl;
      if (!info) {
        newTmpl = new this.Template(noopTmplSrc, this, '', eagerCompile);
      } else {
        newTmpl = new this.Template(info.src, this, info.path, eagerCompile);
        if (!info.noCache) {
  ...
  renderString(src, ctx, opts, cb) {
    ...
    const tmpl = new this.Template(src, this, opts.path);
...
export class BaseTemplate extends Obj {
  ...
  _compile() {
    throw new Error('_compile must be overridden in a subclass of BaseEnvironment');
  }
