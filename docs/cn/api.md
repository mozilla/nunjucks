...
这个方法为了与 Jinja 更好的兼容，增加了一些适配 Python 的 API。但是 govjucks 不是为了完全兼容 Jinja/Pyhton，这只为了帮助使用者查看。
...
在浏览器端使用 govjucks 需要考虑更多，因为需要非常关注加载和编译时间。在服务端，模板一次编译后就缓存在内存中，就不用担心了。在浏览器端，你甚至不想编译他，因为会降低渲染的速度。
